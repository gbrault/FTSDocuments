        ...
        if 'visited' not in box1:
            ...
            for j, box2 in enumerate(boxes):
                if not 'visited' in box2:
                    ...
                    if ( abs(box2["xlt"]-current_box["xrb"]) < epsilon and # current_box is on the left of box2 all left boxes of box2 are already visited this expression is > 0
...
def font_spec(doc):
    ...
    for page in doc.pages:
        ...
        for char in page.objects['char']:
            ...
            if char['fontname'] in all_fonts:
                if size in all_fonts[char['fontname']]:
                    all_fonts[char['fontname']][size]['n'] +=1
                    all_fonts[char['fontname']][size]['w'] += char['width']
                else:
                    all_fonts[char['fontname']][size] = {"n":1, "w":char['width']}
            else:
                all_fonts[char['fontname']]={size:{"n":1, "w":char['width']}}
    for font,specs in all_fonts.items():
        for size, stat in specs.items():
            stat["aw"] = round(stat["w"]/stat["n"],2)
            stat["w"] = round(stat["w"],2)
    return all_fonts

# get bocks of text from pdf file on disk using fitz
def read_doc(doc, all_fonts=None):
    all_rectangles = []
        ...
        for page in doc.pages:
            ...
            for i,char in enumerate(all_chars):
                ...
                if i==0:
                    ...
                    if (groups[-1][j]["y1"] == char["y1"]) and ( #(groups[-1][j]["y0"] == char["y0"])
                        """
                        groups[-1][j]["fontname"] == char["fontname"]"""
                        ) and (
                        abs(groups[-1][j]["size"] - char["size"])<(0.3*(char["size"]))) and (
                        (char["x0"]-groups[-1][j]["x1"]) < 2*all_fonts[char["fontname"]][size]["aw"]):
            ...
            crop_blocks = []
            for group in groups:
                # make text chucks from characters
                l = ""
                for i,char in enumerate(group):
                    size = round(char["size"],2)
                    if i > 0:
                        if (char["x0"]-group[i-1]["x1"])>0.2*all_fonts[char["fontname"]][size]["aw"]:
                            l +=" "
                    l += char["text"]
...
def pdf_to_documentsfilescontent(file):
    ...
    with Session() as session:
        ...
        if fts_config.PDF_BACKEND == "PyMuPDF":
            ...
            all_fonts = font_spec(doc)
            rectangles = read_doc(doc, all_fonts=all_fonts)
            doc.close()
        else:
            logger.error('PDF_BACKEND not supported: please review fts_config.py content') # ça sort pas!
            return
        ...
        for rectangle in rectangles:
            ...
            try:
                ...
                logger.error('Error inserting into FTS_DocumentsFilesContent: '+str(e))
                return
